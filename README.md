...
科研地图按主目标版本展示演进线索，每个版本直接显示建立目标与最终判断，历史任务聚合进版本详情；只有与探索或 catalog 关系关联，或带 `core`/`map` 标签的关键文件会进入地图。搜索页可按任务、探索、决策和资料快速筛选。资料页作为七类标准目录的导航与定位中心，只浏览选中目录，文字检索统一进入搜索页。
