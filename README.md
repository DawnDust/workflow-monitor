...
Dashboard 的“工作台”页按文献研究、研究设计、研究复盘和研究规划分类列出七类提示词。左侧筛选并选择提示词，右侧查看和临时编辑完整内容，再显式复制到 Codex；分析结果只有在用户明确确认后才会按维护规范记录。

Dashboard 的搜索、资料、工作台、任务和记录页统一采用左侧列表、右侧完整详情的分栏布局，便于浏览记录并减少上下滚动。
