...
class WebDashboardIntegrationTests(unittest.TestCase):
    ...
    def test_static_assets_are_local_es_modules_and_packaged(self) -> None:
        ...
        self.assertIn('"Working","Progress recorded"', script)
        self.assertIn('"工作中","进展已记录"', script)
        self.assertIn("Checkpoint is stale. Record progress again.", script)
        self.assertIn("Checkpoint 已过期，请重新记录进展。", script)
