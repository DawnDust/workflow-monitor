...
from ...application.workbench_service import (
    EXTERNAL_TOOL_KINDS, LEGACY_ITEM_TYPES, WORKBENCH_KINDS, WORKBENCH_PURPOSES,
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    item = workbench_sub.add_parser("item")
    item_sub = item.add_subparsers(dest="item_command", required=True)
    item_list = item_sub.add_parser("list")
    item_list.add_argument("--kind", choices=WORKBENCH_KINDS)
    item_list.add_argument("--type", dest="legacy_item_type", choices=LEGACY_ITEM_TYPES, help=argparse.SUPPRESS)
    item_list.add_argument("--purpose", dest="purposes", action="append", choices=WORKBENCH_PURPOSES)
    item_list.add_argument("--status", choices=("active", "paused", "retired"))
    item_list.add_argument("--format", choices=("markdown", "json"), default="markdown")
    item_show = item_sub.add_parser("show")
    item_show.add_argument("item_id")
    item_show.add_argument("--format", choices=("markdown", "json"), default="markdown")
    item_add = item_sub.add_parser("add")
    item_add.add_argument("item_id")
    item_add.add_argument("--kind", choices=WORKBENCH_KINDS)
    item_add.add_argument("--type", dest="legacy_item_type", choices=LEGACY_ITEM_TYPES, help=argparse.SUPPRESS)
    item_add.add_argument("--purpose", dest="purposes", action="append", choices=WORKBENCH_PURPOSES)
    item_add.add_argument("--title", required=True)
    item_add.add_argument("--summary", required=True)
    item_add.add_argument("--path", required=True)
    item_add.add_argument("--tag", dest="tags", action="append")
    item_add.add_argument("--reference")
    item_update = item_sub.add_parser("update")
    item_update.add_argument("item_id")
    item_update.add_argument("--kind", choices=WORKBENCH_KINDS)
    item_update.add_argument("--type", dest="legacy_item_type", choices=LEGACY_ITEM_TYPES, help=argparse.SUPPRESS)
    item_update.add_argument("--purpose", dest="purposes", action="append", choices=WORKBENCH_PURPOSES)
    item_update.add_argument("--review-state", choices=("reviewed", "needs_review"))
    item_update.add_argument("--review-note")
    item_update.add_argument("--title")
    item_update.add_argument("--summary")
    item_update.add_argument("--path")
    item_update.add_argument("--tag", dest="tags", action="append")
    item_update.add_argument("--reference")
    item_update.add_argument("--refresh-content", action="store_true")
    for item_state in ("pause", "restore", "retire"):
        state_parser = item_sub.add_parser(item_state)
        state_parser.add_argument("item_id")
        state_parser.add_argument("--note")
    package = workbench_sub.add_parser("package")
    ...
    package_inspect.add_argument("--format", choices=("markdown", "json"), default="markdown")
    package_import = package_sub.add_parser("import")
    package_import.add_argument("archive")
    package_export = package_sub.add_parser("export")
    package_export.add_argument("package_id")
    package_export.add_argument("--name", required=True)
    package_export.add_argument("--version", required=True)
    package_export.add_argument("--author", required=True)
    package_export.add_argument("--description")
    package_export.add_argument("--license", dest="license_name")
    package_export.add_argument("--item", dest="item_ids", action="append", required=True)
    ...
    end.add_argument("--route", choices=("changed", "unchanged"))
