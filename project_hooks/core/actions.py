...
ACTION_SPECS: dict[str, ActionSpec] = {
    "task.start": ActionSpec(
        ...
        (
            ...
            _f("without_stage_reason", "无需研究篇章理由", "multiline", natural=True),
        ), requires_idle=True,
    ...
    "stage.start": ActionSpec(
        "stage.start", "开始新研究篇章", "项目与研究篇章",
        (
            _f("stage_id", "篇章 ID", required=True, maximum=100),
            _f("title", "篇章标题", required=True, natural=True, maximum=200),
            _f("goal", "篇章目标", "multiline", required=True, natural=True),
            _f("acceptance", "验收条件（每行一项）", "list", required=True, natural=True, maximum=1000),
        ), confirmation_level="preview", requires_active=True, requires_stable=True,
    ),
    "stage.update": ActionSpec(
        "stage.update", "更新当前研究篇章", "项目与研究篇章",
        (
            _f("stage_id", "篇章 ID", required=True, maximum=100),
            _f("summary", "篇章概况", "multiline", natural=True),
            _f("current_step", "当前步骤", "multiline", natural=True, maximum=500),
            _f("next_step", "下一步", "multiline", natural=True, maximum=500),
            _f("blocker", "篇章阻塞", "multiline", natural=True),
            _f("evidence", "篇章证据（每行一项）", "list", natural=True, maximum=1000),
            _f("status", "篇章状态", "choice", choices=("", "active", "completed", "paused", "cancelled"), default=""),
            _f("revision", "判断修订说明", "text", required=False),
            _f("pause_kind", "暂停类型", "choice", choices=("", "interruption", "temporary-closure"), default=""),
            _f("pause_note", "暂停或暂时收束说明", "text", required=False),
        ), confirmation_level="preview", requires_active=True, requires_stable=True,
    ),
