...
2. 在首次写入前运行 `python -m project_hooks start <task_id> ...`，保存文件与 Git 基线。
3. 完成实现、验证和必要文档更新；失败、阻塞和不可判决同样记录。
4. 在 `CHANGE_ARCHIVE.md` 追加一条任务记录；若路线或架构改变，再更新 `DECISION_LOG.md`。
5. 最后更新 `CURRENT_TASK.md`，只保留当前判决、真实断点、最多三步和最近五次交接。
6. 运行 `python -m project_hooks end <task_id> ...`。未满足义务时不得绕过门禁。
...
- 第一次安装通过 `python -m project_hooks install` 设置仓库本地 `pre-commit` 门禁。
