...
- `python -m project_hooks dashboard` 打开 Tkinter 窗口，显示概览、全局搜索、Git 时间线、任务历史、决策、探索和原始事件。
- 时间线读取本地 Git DAG 和 `events.jsonl` 的提交历史：分支泳道显示全部可达提交，实线表示 Git 父子关系，虚线表示任务事件的跨分支关联；不会访问 GitHub 网络。
- 全局搜索覆盖任务、决策、探索和提交；快捷筛选固定为最近 10 条任务、`negative` 探索和未合并的活动探索分支，原始事件不参与全局搜索。
- 时间线对当前视图默认保留最近 50 个提交，旧提交以摘要块折叠，可临时展开全部；搜索定位旧提交时自动展开。
- 窗口支持分支筛选、本页高亮、排序、复制、手动刷新和自动刷新；不直接写数据库、不修改事件日志，也不执行 Git 操作。
